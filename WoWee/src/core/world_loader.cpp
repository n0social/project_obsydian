...
void WorldLoader::processPendingEntry() {
    ...
    if (renderer_ && renderer_->getCameraController()) {
        ...
        renderer_->getCameraController()->suspendGravityFor(
#ifdef __ANDROID__
            30.0f  // Tablet ADT I/O is slow; keep gravity off until tiles land.
#else
            10.0f
#endif
        );
    ...
    if (isWMOOnlyMap) {
        ...
        if (terrainOk && renderer_->getTerrainManager() && renderer_->getCamera()) {
...
#ifdef __ANDROID__
            // Tab A9+ class devices stall TCP/VPN if we block on a huge ADT fan-in.
            const int savedLoadRadius = 4;
            terrainMgr->setLoadRadius(2);   // 5x5 = 25 tiles
            terrainMgr->setUnloadRadius(6);
#else
            ...
            while (terrainMgr->getRemainingTileCount() > 0) {
                ...
                // Keep the world socket alive (Warden + keepalive) while ADTs load.
