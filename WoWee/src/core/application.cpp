...
bool Application::initialize() {
    ...
    // Peek display pacing from settings.cfg before the window exists so the
    // first swapchain matches launcher/desktop preferences.
    bool settingsVsync = true;
#ifdef __ANDROID__
    bool settingsFullscreen = true;
#else
    bool settingsFullscreen = false;
#endif
    {
        std::ifstream cfg(ui::SettingsPanel::getSettingsPath());
        std::string line;
        while (cfg && std::getline(cfg, line)) {
            ...
            if (key == "vsync") settingsVsync = (val == "1" || val == "true");
            else if (key == "fullscreen") settingsFullscreen = (val == "1" || val == "true");
        }
    }
...
#ifdef __ANDROID__
    // Galaxy Tab A9+ (and similar) native landscape. SDL fullscreen-desktop
    // will snap to the real display; requesting landscape dims keeps Android's
    // orientation helper from locking portrait from the physical 1200x1920.
    windowConfig.width = 1920;
    windowConfig.height = 1200;
    windowConfig.fullscreen = settingsFullscreen;
    ...
    windowConfig.vsync = settingsVsync;
    ...
    if (assetManager->initialize(assetPath)) {
        ...
        if (gameHandler && gameHandler->getTransportManager()) {
            ...
            // Still aborts under Bionic FORTIFY during TransportAnimation parse on
            // Adreno (log cuts mid "Loaded N transport..."). Keep skipped; ferries
            // use fallbacks. Settings/auth/UI path above is verified working.
            LOG_WARNING("Skipping TransportAnimation/TaxiPathNode DBC load on Android");
#else
            LOG_INFO("Loading TransportAnimation/TaxiPathNode DBC...");
            ...
            LOG_INFO("Transport DBC load finished");
#endif
        }

        // Initialize addon system
        LOG_INFO("Initializing addon system...");
