...
void CharacterScreen::render(game::GameHandler& gameHandler) {
    ...
    if (characters.empty() &&
        ...
        ImGui::TextWrapped(
            "The world connection closed. You can go back and retry, or open "
            "character creation if this account has no characters yet.");
        ImGui::Spacing();
        if (ImGui::Button("Back", layout.button(140.0f))) { if (onBack) onBack(); }
        ImGui::SameLine(0.0f, layout.gap());
        if (ImGui::Button("Create Character", layout.primaryButton(220.0f))) {
    ...
    if (characters.empty()) {
#ifdef __ANDROID__
        // Only auto-open create after a real empty enum — never while the list
        // is still loading or after a disconnect cleared characters.
        if (onCreateCharacter && !androidCreateAutoOpened_ &&
            gameHandler.getState() == game::WorldState::CHAR_LIST_RECEIVED) {
            androidCreateAutoOpened_ = true;
            onCreateCharacter();
            ImGui::End();
            return;
        }
#endif
        ImGui::Text("No characters on this account.");
        ImGui::TextDisabled("Create one to enter the world.");
        ImGui::Spacing();
        if (ImGui::Button("Back", layout.button(140.0f))) { if (onBack) onBack(); }
        ImGui::SameLine(0.0f, layout.gap());
        if (ImGui::Button("Refresh", layout.button(140.0f))) {
            if (gameHandler.getState() == game::WorldState::READY ||
                gameHandler.getState() == game::WorldState::CHAR_LIST_RECEIVED) {
                gameHandler.requestCharacterList();
    ...
    if (selectedCharacterIndex >= 0 &&
        ...
        if (disconnected) {
            if (statusMessage.empty() || !statusIsError) {
                setStatus("Disconnected from world server — reconnect to enter.", true);
            }
            ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.45f, 0.28f, 0.08f, 1.0f));
            ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(0.58f, 0.36f, 0.10f, 1.0f));
            if (ImGui::Button("Reconnect", primarySize)) {
                setStatus("Returning to reconnect...");
                if (onBack) onBack();
            }
