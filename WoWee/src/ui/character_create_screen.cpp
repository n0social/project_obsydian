...
void CharacterCreateScreen::render(game::GameHandler& /*gameHandler*/) {
...
#ifdef __ANDROID__
        // Give the portrait preview most of the landscape width; controls keep the rest.
        ? std::max(640.0f * layout.scale, availableWidth * 0.58f)
#else
    ...
    if (hasPreview) {
        ...
        );
        {
            ...
            const float availH = std::max(200.0f,
                ImGui::GetContentRegionAvail().y - 36.0f * layout.scale);
            const float texAspect = static_cast<float>(preview_->getHeight()) /
                                    static_cast<float>(preview_->getWidth());
            // Fit inside the panel without stretching — non-uniform scale was
            // warping limbs (ankles/arms looked cut and shoved inward).
            float imgW = availW;
            ...
            const float indentX = (availW - imgW) * 0.5f;
            const float indentY = (availH - imgH) * 0.5f;
            if (indentX > 0.0f) ImGui::SetCursorPosX(ImGui::GetCursorPosX() + indentX);
            if (indentY > 0.0f) ImGui::SetCursorPosY(ImGui::GetCursorPosY() + indentY);
...
#ifdef __ANDROID__
    // First open: focus name so the soft keyboard comes up immediately.
