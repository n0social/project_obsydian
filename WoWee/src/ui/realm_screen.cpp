    ...
    if (realms.empty()) {
        ...
        if (!autoSelectAttempted && !realmSelected) {
            ...
            if (realms.size() == 1 && !realms[0].lock) {
                ...
                // Pre-highlight realm with characters (don't auto-connect — let
                // the player confirm). Skipping the list hid realms and made
                // Enter World look broken when char enum was still in flight.
