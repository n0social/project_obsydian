...
void GameHandler::resetWardenState() {
    ...
    // Cheat-check handling lives on WardenHandler (separate RC4 state). Always
    // reset it on connect/disconnect or the next session decrypts with a stale key.
    if (wardenHandler_) {
...
void GameHandler::updateNetworking(float deltaTime) {
    ...
    // Legacy GameHandler-owned async drain (kept for any residual callers).
