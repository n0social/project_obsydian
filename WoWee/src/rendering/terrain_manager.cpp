...
#ifdef __ANDROID__
            // Tablets often report <15% MemAvailable on a 3 GB device even when
            // terrain must load. Percentage gates left workers sleeping forever
            // → 0 ADTs → endless void fall. Only pause on critically low headroom.
            constexpr size_t kAndroidCriticalBytes = 64ull * 1024 * 1024;
            const size_t avail = memMon.getAvailableRAM();
            if (avail > 0 && avail < kAndroidCriticalBytes) {
                lock.unlock();
