    ...
    for (auto& pair : instances) {
        ...
        if (!gpuModel.data.batches.empty()) {
            ...
            for (size_t bi : sortedBatchIndices) {
                ...
                // Missing glue-scene BLPs become a stretched 1×1 white quad (the
                // classic "big square" over Tauren / holey Stormwind walls). Skip
                // those batches instead of advertising the miss as a billboard.
                if (instance.isSceneModel && texPtr == whiteTexture_.get()) {
                    continue;
                }
